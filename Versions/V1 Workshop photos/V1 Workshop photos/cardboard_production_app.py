...
class CardboardProductionApp:
    def __init__(self, barcode: str = "1234567890", video_source: str = "0", qwen_model: str = "qwen2.5vl:7b", yolo_device: str = "cpu"):
        import torch
        ...
        # Configure Ollama to use RTX GPU (device 0)
        os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # Only show RTX GPU to Ollama
    ...
    def analyze_with_qwen(self, frame_with_boxes: np.ndarray, picture_number: int) -> Dict:
        """Analyze frame with Qwen and return structured result"""
        print(f"\n🔍 Analyzing Picture {picture_number}...")
...
prompt="You are an expert cardboard quality inspector. Analyze the cardboard pieces within the green bounding boxes in this image.\n\nFocus on:\n1. WARP: Look for major bending, curving, or deformation from a flat surface, slight bending is acceptable.\n2. QUALITY: Assess overall condition including:\n   - Surface damage (tears, holes, dents)\n   - Edge integrity and straightness\n   - Color uniformity and staining\n   - Structural soundness\n\nReturn ONLY this exact JSON format with no additional text:\n{\"warp\": true, \"quality\": \"good\"}\n\nwarp: true if ANY major bending/curving, false if completely flat or slight bending\nquality: \"good\" (excellent condition), \"medium\" (minor defects), \"bad\" (significant damage) \n If cardboard is not visible, return \"unknown\" in both warp and quality. If warp is true, return \"bad\" in quality.",
                images=[img_base64],
                stream=False,
                options={
    ...
    def run_production_analysis(self):
        ...
        try:
            while self.current_picture < self.total_pictures:
                ...
                # Show live feed
                cv2.imshow('Cardboard Production Analysis', frame_display)
                ...
                if key == ord('\r') or key == 13:  # Enter key
                    if results[0].boxes is not None and len(results[0].boxes) > 0:
                        ...
                        filename = f"capture_{self.barcode}_pic{self.current_picture + 1}_{timestamp}.jpg"
                        cv2.imwrite("./capture/"+filename, frame_for_analysis)
                        print(f"📸 Captured: {filename}")
                        
                        # Analyze with Qwen
                        analysis_result = self.analyze_with_qwen(frame_for_analysis, self.current_picture + 1)
...
def main():
    parser = argparse.ArgumentParser(description='Cardboard Production Analysis System')
    ...
    parser.add_argument('--qwen-model', type=str, default='gpt-oss:20b',
                       help='Qwen model to use (default: qwen2.5vl:7b)')
